...
class call_diversity(APIView):
    def post(self, request):
        try:
            ...
            mean_dist_standardized = np.mean(
                pdist(prop[idx_abs])
            )  # mean Euclidean distance of standardized property
            mean_dist_raw = np.mean(
                pdist(prop_raw[idx_abs])
            )  # mean Euclidean distance of raw property
