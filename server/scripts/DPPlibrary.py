...
def RFF(data, D_feature):
    ...
    # D_feature x D_data
    Ws = np.random.normal(size=(int(D_feature / 2), D_data))
...
def sample_dual_dpp(B, C, k=batch_size):
    ...
    for i in range(k - 1, -1, -1):
        ...
        if not len(tmp) == 0:
            ...
            raise ValueError(
                "couldnt find a valid index with rand#="
                + str(np.round(tmp_rnd, 3))
                + ". "
                + "cumsum(P).max() is "
                + str(np.round(P.max(), 3))
            )
        ...
        for a in range(i - 1):
            for b in range(a - 1):
                V[:, a] = (
                    V[:, a]
                    - (np.expand_dims(V[:, a], axis=-1).T @ C["M"] @ V[:, b]) * V[:, b]
                )
